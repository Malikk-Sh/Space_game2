...
function zorpGraph(){
  ...
  return{type:'graph',start:'intro',speaker:'ЗОРП',nodes:{
    ...
    join:{text:['ЗОРП - В КОМАНДЕ!','ПЕДАЛИ - ЭТО ХОРОШО.','СКАЖИ КЛИРРУ, ЧТО Я УШЁЛ.','ОН РАССТРОИТСЯ, НО ПЕРЕЖИВЁТ.'],end:true,effect:recruit},
    later:{text:['ЗОРП:','ОК. Я БУДУ ВОН ТАМ.','И ВОН ТАМ.','И СНОВА ВОН ТАМ.'],end:true},
    bye:{text:['ЗОРП:','БЕГИ. Я ТОЖЕ ПОБЕЖАЛ.'],end:true},
    repeat:{text:['ЗОРП:','Я УЖЕ НА БОРТУ - НЕ ОТВЛЕКАЙ.','...ЭТО ШУТКА. Я ЗДЕСЬ. ПЕДАЛИ ЖДУ.'],end:true},
  }};
}
function krokGraph(){
  return{type:'graph',start:'intro',speaker:'КРОК',nodes:{
    ...
    repeat:{text:['КРОК:','ТЫ ЕЩЁ ЗДЕСЬ.','ЧИТАЙ ЗАПИСИ. ОСОБЕННО ГЛАВУ СЕМЬ.','НЕ ГЛАВУ ВОСЕМЬ. ОНА ПРО ВЕРОЯТНОСТИ.','НЕПРИЯТНАЯ.'],end:true},
  }};
}
