...
function drwHUD(G){const p=G.pl;rc(0,0,LW,16,P.UIB);rc(0,15,LW,1,P.DIM);txt('ХП',2,3,P.UIT2,1);bar(14,3,42,5,p.hp/p.mhp,p.hp/p.mhp<.3?P.HPH:P.HP,P.HPB,P.DIM);txt('ЭН',2,10,P.UIT2,1);bar(14,10,42,4,p.en/p.men,p.en<20?P.ENL:P.EN,P.DIM2,P.DIM);let ix=58;if(p.shield>0){const a=.6+.4*Math.sin(G.sT*.2);cx.globalAlpha=a;ring(ix+2,5,3,P.CYA,1);cx.globalAlpha=1;bar(ix,9,5,1,p.shield/600,P.CYA,P.DIM2);ix+=8;}const pbx=Math.max(62,ix),pby=4,pbw=140-(ix-62);rc(pbx,pby,pbw,6,P.SCAN);const fw=Math.floor(pbw*G.prog);rc(pbx,pby,fw,6,P.PL1);if(fw>1)rc(pbx,pby,fw,1,P.PL2);cx.strokeStyle=P.DIM;cx.lineWidth=.5;cx.strokeRect(pbx+.5,pby+.5,pbw-1,5);const boostPix=(G.pl.boost>0)?2:0;const shipPx=pbx+Math.max(0,Math.min(pbw-3,(fw-3)+boostPix));rc(shipPx,pby,3,1,P.CYA);rc(shipPx+1,pby+1,2,1,P.WHT);rc(shipPx,pby+2,3,1,P.CYA);const destInfo=PLANETS[G.campaignState.targetPlanet]||PLANETS.drosh;txt(destInfo.name,pbx+pbw+2,pby-1,P.PL2,1);if(G.combo>1){const cxx=pbx+pbw+2;txs('x'+G.combo,cxx,pby+5,G.combo>=5?P.YEL:P.UIT2,P.BLK,1);}const wc=[P.L1,P.L3],wLabels=['L1','L2'],wcost=[8,55];const strongUnlocked=G.campaignState.inventory.laserStrong;if(!USE_TOUCH_UI){for(let w=1;w<=2;w++){const wx=pbx+(w-1)*20,wy=10,act=p.wep===w;const unlocked=(w===1)||strongUnlocked;const canAfford=unlocked&&p.en>=wcost[w-1];bx2(wx,wy,18,5,act?'#003344':(canAfford?'#001122':'#110000'),act?wc[w-1]:(canAfford?P.DIM:'#441111'));txt(wLabels[w-1],wx+4,wy,unlocked?(act?P.WHT:(canAfford?wc[w-1]:'#664444')):'#663333',1);}txt('КР:'+p.cr,pbx+pbw+26,2,P.YEL,1);txt('РЕ:'+p.res,pbx+pbw+26,9,P.RES,1);}else{txt('КР:'+p.cr,pbx,10,P.YEL,1);txt('РЕ:'+p.res,pbx+34,10,P.RES,1);const wx=pbx+76,wy=10;bx2(wx,wy,18,5,'#003344',wc[Math.min(1,p.wep-1)]);txt(wLabels[Math.min(1,p.wep-1)],wx+4,wy,P.WHT,1);}if(G.ship){const fy=18;const fc=G.ship.fuel<20?P.RED:P.ORA;cx.globalAlpha=.92;rc(2,fy,54,7,P.UIB);txt('ТОПЛ',4,fy+1,P.UIT2,1);bar(25,fy+1,28,4,G.ship.fuel/100,fc,P.DIM2,P.DIM);if(G.ship.fuel<1&&Math.floor(G.sT/12)%2)txs('АВАР.ХОД',58,fy+1,P.RED,P.BLK,1);cx.globalAlpha=1;}drwWorkerHUD(G);}
